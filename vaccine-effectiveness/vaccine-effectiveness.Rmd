...
options(reproduce.at="2022-06-10")
...
omicronVsControl = 
  bind_rows(
    tmp %>% mutate(odds_ratio_ve(vaccinatedCase = roll_omicron_2vacc,unvaccinatedCase = roll_omicron_novacc,vaccinatedControl = roll_control_2vacc, unvaccinatedControl = roll_control_novacc)) %>% mutate(doses = "two doses"),
...
p = ggplot(plotData, aes(x=admission.date, y=VE*100, ymin=VE.q.0.025*100, ymax=VE.q.0.975*100))+geom_line(aes(colour=doses))+geom_ribbon(aes(fill=doses),alpha=0.2)+coord_cartesian(ylim=c(-50,100))+facet_wrap(vars(comparison),ncol=1)+ylab("VE (%)")
