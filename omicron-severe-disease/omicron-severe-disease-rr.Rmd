...
avoncap_raw = load_data("DeltaVE") 
avoncap_raw %>% save_data_source_info(out)
...
excluded = raw_data %>% excluded() 
excluded %>% openxlsx::write.xlsx(file = out("exclusions.xlsx"))
...
#   geom_bar(stat="identity",position=position_dodge(width = 0.7),width=0.7, alpha=0.2)+
#   geom_errorbar(colour="black", position=position_dodge(width = 0.7), width=0.5)+
#   ylab("Cumulative proportion")+xlab("Max FiO2 level less than or equal to")+
#   scale_color_brewer(palette = "Dark2",name="",aesthetics = c("fill","colour"))
# 
...
p1 = raw_data %>% plot_outcome(day_7.max_o2_gt_28 == v$day_7.max_o2_gt_28$`over 28%`,"% on >28% FiO2",stratifyBy = demog.age_category)+gg_hide_legend()+gg_set_X_angle(45)
...
# N.b. this actually selects out
mnar = "demog.imd_decile"
...
captions = list(
  ...
  table4 = "Univariate and multivariate robust poisson regression coefficients for the comparison of the binary outcome {outcome}. The univariate effects of each predictor are shown in the first column, the multivariate adjustment of all variables together, excluding the QCovid2 score is in the second column. The third column is a regression model focussing on the key variables, vaccination status, genomic variant, and the QCovid2 score. The fourth column is a regression model focussing on the key variables, vaccination status, genomic variant, Charlson comorbidity score, and age as a categorical value.",
...
footnotes = list(
  table1 = "bp, blood pressure; CCI, charlson comorbidity index; CXR, chest Xray; IMD, index multiple deprivation; ivdu, intravenous drug usage; sd, standard deviation; T, temperature",
  table2 = "bp, blood pressure; CCI, charlson comorbidity index; CXR, chest Xray; IMD, index multiple deprivation; ivdu, intravenous drug usage; no, number; sd, standard deviation; T, temperature",
  table3 = NULL,
...
# Outcome by outcome regression models
...
additional = as.formula("~ . + demog.gender + demog.ethnicity + admission.on_immunosuppression + demog.care_home_resident + admission.duration_symptoms + admission.week_no")
...
multiConfig = tibble::tribble(
  ~modelName, ~form,
  "Max FiO2>28%", (day_7.max_o2_gt_28 ~ .) %>% update(baseCCIcat),
...
# this is table 2?
h2 = multivModels %>% 
  mutate(modelName = modelName %>% ordered(c("Max FiO2>28%","WHO Outcome score>5","LOS>3 days"))) %>% 
...
out1 = multivModels2 %>% mutate(modelName = modelName %>% ordered(c("Max FiO2>28%","WHO Outcome score>5","LOS>3 days"))) %>% combine_boots(predictors2)
# out1 = multivModels %>% mutate(modelName = modelName %>% ordered(c("Max FiO2>28%","WHO Outcome score>5","LOS>3 days"))) %>% combine_boots(predictors2)
perf = multivModels %>% mutate(modelName = modelName %>% ordered(c("Max FiO2>28%","WHO Outcome score>5","LOS>3 days"))) %>% pool_performance(statistics = c("AIC","BIC","R2_Tjur","R2_Nagelkerke","RMSE","Sigma"))
...
p1_alt = ggplot(longer, aes(y=y, x=x,xmin=xmin,xmax=xmax,colour=modelName))+
  geom_tile(mapping=aes(fill=as.character(g),x=1),width=Inf,colour=NA)+
  geom_point(position=ggstance::position_dodgev(height=0.5,preserve = "total"),size=0.4)+
  geom_errorbarh(position=ggstance::position_dodgev(height=0.5,preserve = "total"),height=0.4)+ 
  scale_x_log10(breaks = c(0.25,0.5,1,2,4)) +
...
if (longer %>% filter(label==as.character(group) & !is.na(p.value)) %>% nrow() > 0) {
  ...
  p1_alt = p1_alt+geom_text(data = longer %>% filter(label==as.character(group) & !is.na(p.value)) %>% mutate(label=paste0("(p: ",p.value,")")), mapping=aes(y=y,x=4,label=label),inherit.aes = FALSE,size=gg_label_size(5),hjust=1,colour="black")
}
...
p = p2+p1_alt+patchwork::plot_layout(ncol=2,nrow=1,widths = c(2.5,10))
p %>% gg_save_as(out("fig4-forest-plot-no-p-indicator"), size = std_size$third)
...
multiConfig = tibble::tribble(
  ~modelName, ~form,
  "Max FiO2>28%", (day_7.max_o2_gt_28 ~ .) %>% update(crossCCIcat),
...
multiConfig = tibble::tribble(
  ~modelName, ~form,
  "Max FiO2>28%", (day_7.max_o2_gt_28 ~ .) %>% update(baseCCIcat),
  "Max FiO2>35%", (day_7.max_o2_gt_35 ~ .) %>% update(baseCCIcat),
  "Max FiO2>50%", (day_7.max_o2_gt_50 ~ .) %>% update(baseCCIcat),
)
...
h2 = multivModels %>% 
  mutate(modelName = modelName %>% ordered(c("Max FiO2>28%","Max FiO2>35%","Max FiO2>50%"))) %>% 
  summarise_boots(predictors,"RR")
...
# N.b. shoudl be a better way of doing this but I calculated all the additional data points in the 

full_data = bind_rows(
  avoncap_original %>% semi_join(excluded %>% filter(.stage != "standard exclusions") %>% select(admin.record_number), by="admin.record_number") %>% mutate(status = "excluded"),
  avoncap_original %>% semi_join(raw_data %>% ungroup() %>% select(admin.record_number), by="admin.record_number") %>% mutate(status = "included")
)



tmp = full_data %>% compare_population(
  populationVar = status,
  comparisonVars = vars(
    genomic.variant_inferred,
  demog.age,
  demog.gender,
...
h %>% hux_auto_widths() %>%
  huxtable::map_contents(huxtable::everywhere,huxtable::everywhere,huxtable::by_function(function(x) stringr::str_replace_all(x,fixed("."), "\u00B7"))) %>%
  sup$add_table(
    caption="Comparison of major demographic features between included and excluded cases. Vaccination status is not included as that is a criteria for exclusion.",
    footnote = footnotes$table4,
    index=19)
  
```

```{r S20_inferred_variants}

# N.b. shoudl be a better way of doing this but I calculated all the additional data points in the 

inferred_variants = raw_data %>% mutate(inferred = ifelse(!is.na(genomic.variant_inferred) & is.na(genomic.variant),"variant inferred","variant known"))

tmp = inferred_variants %>% compare_population(
  populationVar = inferred,
  comparisonVars = vars(
    genomic.variant_inferred,
  demog.age,
  demog.gender,
...
h %>% hux_auto_widths() %>%
  huxtable::map_contents(huxtable::everywhere,huxtable::everywhere,huxtable::by_function(function(x) stringr::str_replace_all(x,fixed("."), "\u00B7"))) %>%
  sup$add_table(
    caption="Comparison of major demographic features between cases with inferred and known vaccination statuses. Inference is based on the two times, firstly before which there was no Omicron observed, and secondly after which there was no Delta observed. Inferred cases therefore have a strong temporal element to them and this leads to significant differences in the populations of the variant inferred and variant known categories that have no relevance to the study's findings, but mainly reflect the fact that more Omicron cases were inferred and hence inferred cases demonstrate the similar patterns as the main analysis.",
    footnote = footnotes$table4,
    index=20)
