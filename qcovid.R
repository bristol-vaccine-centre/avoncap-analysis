avoncap_calculate_qcovid = function(avoncap_df) {
    ...
    qcovid2.log_hazard = ifelse(
      ...
      # Female
        .positive_death_female(
          ...
          chemocat = rep(1,length(admission.BMI)), # integer ?chemotherapy
          ethrisk = case_when(
            # 1 Unknown
            # 2 White
  ...
  out = out %>% mutate(qcovid2.hazard_ratio = exp(qcovid2.log_hazard))
}
  ...
  )
{
  ...
  age_1 = dage^3;
  dbmi = ifelse(!is.finite(bmi), 25, bmi);
  dbmi = dbmi/10;
  bmi_2 = log(dbmi)^2;
  bmi_1 = log(dbmi);
...
.female_survival = function(a,surv) {
  survivor = c(
    0,
    0,
    0,
